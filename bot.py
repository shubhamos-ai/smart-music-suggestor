...
class ModerationBot(discord.Client):
    def __init__(self, intents: discord.Intents) -> None:
        # Change to whatever prefix you want - removing the space to make it easier to use
        self.prefix = "devil"
        self.prefix_length = len(self.prefix)
        print(f"Bot prefix set to: '{self.prefix}'") # Debug message
    ...
    async def event_template(self, *args, **kwargs) -> None:
        ...
        print(f"DEBUG: Event triggered: {event_name}")
        
        # Special debug for message events
        if event_name == "on_message" and args:
            message = args[0]
            print(f"DEBUG: Message received: {message.content}")
            
            # Prevent handling your own messages to reduce unnecessary processing
            if message.author == self.user:
                return
        
        # Get cached handlers instead of creating new ones each time
        event_handlers = self.event_registry.get_event_handlers(event_name)
        if event_handlers is not None:
            for event_handler in event_handlers:
                # Create a new handler instance only once per event
    ...
    async def setup_hook(self) -> None:
        """Set bot to invisible mode when connecting"""
        try:
            # Make sure we only try to change presence when the bot is ready
            if self.is_ready():
                await self.change_presence(status=discord.Status.invisible)
                print("Set bot status to invisible in setup_hook()")
            else:
                print("Bot not ready yet, skipping presence change in setup_hook")
        except Exception as e:
    ...
    async def on_ready(self) -> None:
        ...
        try:
            await self.change_presence(status=discord.Status.invisible)
            print(f"{self.user} is now in invisible mode!")
        except Exception as e:
            print(f"Error setting invisible mode: {e}")
